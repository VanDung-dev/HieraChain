...
1. Bật/tắt BFT (nếu áp dụng luồng sắp xếp/Byzantine):
...
1. Khởi động API server và xác minh luồng cơ bản hoạt động:

```bash
python -m hierachain.api.server
```

1. Kiểm thử nhanh bằng API v1:
...
1. Wiring điểm khởi tạo (factory/điểm tích hợp):
...
1. Kiểm thử với API v1 như phần A (thêm event → finalize → submit proof). Theo dõi log để xác nhận phương thức `propose/validate/commit` mới được gọi.
