#!/bin/bash
# HieraChain Rogue (node5) Security Test Runner - Podman Compose
# Runs only the adversarial/security tests against a rogue node (node5) using Podman
...
echo "========================================"
echo " HieraChain Podman Compose Security (node5)"
echo "========================================"
...
if [ "${REUSE_EXISTING_CLUSTER}" != "true" ]; then
  echo "[1/6] Building Podman images..."
  podman compose -f "$COMPOSE_FILE" build
  echo "  ✅ Build complete"
else
  echo "[1/6] Skipping Podman build (REUSE_EXISTING_CLUSTER=true)"
fi
...
if [ "${REUSE_EXISTING_CLUSTER}" != "true" ]; then
  ...
  if [ "${FORCE_RECREATE}" = "true" ]; then
    ...
    podman volume rm -f ipfs-node1-data ipfs-node2-data ipfs-node3-data ipfs-node4-data rogue-node-data 2>/dev/null || true
  fi
  podman compose -f "$COMPOSE_FILE" up -d node1 node2 node3 node4 gateway redis ipfs-node1 ipfs-node2 ipfs-node3 ipfs-node4
else
...
podman compose -f "$COMPOSE_FILE" --profile security-test up -d rogue-node
...
until podman compose -f "$COMPOSE_FILE" exec -T rogue-node \
    ...
    if [ $RETRY_COUNT -ge $MAX_RETRIES ]; then
        ...
        podman compose -f "$COMPOSE_FILE" logs --tail=200 rogue-node || true
        exit 1
    fi
done
...
echo "========================================"
echo "Security report: log/report/podman_rogue_security_report.html"
