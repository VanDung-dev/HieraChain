...
if [ "${REUSE_EXISTING_CLUSTER}" != "true" ]; then
  ...
  if [ "${FORCE_RECREATE}" = "true" ]; then
    ...
    podman compose $COMPOSE_FILES down 2>/dev/null || true
    podman volume rm -f hierachain_ipfs-node1-data hierachain_ipfs-node2-data hierachain_ipfs-node3-data hierachain_ipfs-node4-data 2>/dev/null || true
  fi
...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
    ...
    for node in node1 node2 node3 node4; do
        if podman compose $COMPOSE_FILES exec -T "$node" \
    ...
    for ipfs_node in ipfs-node1 ipfs-node2 ipfs-node3 ipfs-node4; do
        if ! podman compose $COMPOSE_FILES exec -T "$ipfs_node" \
    ...
    echo "  Waiting... ($((RETRY_COUNT + 1))/$MAX_RETRIES, $HEALTHY/4)"
    sleep 3
...
if [ $RETRY_COUNT -ge $MAX_RETRIES ]; then
    ...
    podman compose $COMPOSE_FILES logs --tail=20 || true
    exit 1
fi
...
podman compose $COMPOSE_FILES run --no-deps --rm stress-tester \
    bash -c "
