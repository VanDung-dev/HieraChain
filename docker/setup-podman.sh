...
# Creates a 4-node cluster using Podman Compose for stress testing
...
# Pre-pull public images to avoid Docker credential helper issues with podman-compose/docker-compose
...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
    ...
    # Note: Podman compose might use different port mapping or rootless behavior
...
if [ "$HEALTHY" = false ]; then
    echo "  ❌ Cluster failed to start properly. Check logs with: podman compose -f $COMPOSE_FILE logs"
fi

# Summary
echo ""
echo "========================================"
echo " Deployment complete (Podman)!"
echo "========================================"
echo ""
echo ""
...
echo "  Get token: podman exec hierachain-gateway-podman env | grep EXPLORER_TOKEN"
echo ""
