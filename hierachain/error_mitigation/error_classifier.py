...
class ErrorClassifier:
    ...
    def __init__(
        self, config: dict[str, Any], lockdown_callback: Any | None = None
    ) -> None:
        """
    ...
    def classify_error(self, error_data: dict[str, Any]) -> ErrorInfo:
        ...
        logger.info(
            "Error classified: %s -> %s (%s)", error_id, category.value, priority.name
        )
        return error_info
...
def _assess_impact(error_data: dict[str, Any], category: ErrorCategory) -> ImpactLevel:
    ...
    # Impact assessment handlers for different categories
    handlers = {
...
def _assess_consensus_impact(error_type: str) -> ImpactLevel:
    """Assess impact for consensus errors"""
...
def _assess_security_impact(error_type: str) -> ImpactLevel:
    """Assess impact for security errors"""
...
def _assess_performance_impact(error_type: str) -> ImpactLevel:
    """Assess impact for performance errors"""
...
def _assess_storage_impact(error_type: str) -> ImpactLevel:
    """Assess impact for storage errors"""
    is_critical = ("backup" in error_type and "failed" in error_type) or (
        "corruption" in error_type
    )
    if is_critical:
        return ImpactLevel.MAJOR
    return ImpactLevel.MODERATE
...
def _assess_network_impact(error_type: str) -> ImpactLevel:
    """Assess impact for network errors"""
...
def _assess_likelihood(
    _error_data: dict[str, Any], category: ErrorCategory
) -> LikelihoodLevel:
    """
...
def _log_classification(error_info: ErrorInfo) -> None:
    ...
    logger.info(
        ...
        error_info.priority.name,
        error_info.impact.name,
        error_info.likelihood.name,
    )
    log_entry = {
    ...
    try:
        ...
        with open(
            "log/error_mitigation/error_classifications.log", "a", encoding="utf-8"
        ) as f:
...
def classify_error_quick(
    error_type: str, message: str, config: dict[str, Any] | None = None
) -> ErrorInfo:
    """
