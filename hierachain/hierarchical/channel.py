...
def _format_details(val: Any) -> dict[str, str]:
    """Format details field for Arrow schema."""
...
def _filter_block_events(
    ...
    """Filter events in a single block."""
...
def _build_single_filter(field: str, param: str, value: Any) -> Any:
    """Build a single Arrow filter expression."""
    if param == "start_time":
...
def _check_details_match(event: dict[str, Any], key: str, value: Any) -> bool:
    """Check if event details match query criteria."""
...
def _evaluate_operators(actual_val: Any, operators: dict[str, Any]) -> bool:
    """Evaluate comparison operators against an actual value."""
    try:
...
def _check_event_match(event: dict[str, Any], key: str, value: Any) -> bool:
    """Check if a single query parameter matches an event."""
    if key == "event_type":
...
def _create_query_filter(query_params: dict[str, Any]):
    """Create a Python filter function for complex matching."""
...
class ChannelLedger:
    ...
    def _prepare_event_data(self) -> dict[str, list[Any]]:
        """Prepare arrays for Arrow Table from current events."""
