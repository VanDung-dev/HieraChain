...
def _entry_matches_query(
    ...
    """Check if a private data entry matches the query criteria."""
    return (
        _check_submitter(entry, query_params)
        and _check_timestamp(entry, query_params)
        and _check_block_height(entry, query_params)
    )
...
class PrivateCollection:
    ...
    def query_keys(self, query_params: dict[str, Any], requester_org_id: str) -> list[str]:
        ...
        matching_keys = [
            ...
            if not self._should_purge_entry(entry)
            and _entry_matches_query(entry, query_params)
        ]
    ...
    def __str__(self) -> str:
        ...
        return (
            f"PrivateCollection(name={self.name}, "
